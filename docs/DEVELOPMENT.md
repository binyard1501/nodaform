...
## 위저드 단계 축소 · 자동 임시저장 (구현 완료)

8단계 고정 위저드가 이탈의 가장 큰 원인이었다. 두 가지를 적용했다.

**1. 필요 없는 단계는 숨긴다.** `Wizard.tsx`의 `ALL_STEPS`는 `{ key, label, show(state) }` 배열이고
매 렌더마다 `show`로 걸러 낸다.

- `가격과 정원`·`신청 규칙`·`자동 알림`: `kind !== 'survey'`일 때만
- `결제·환불`: 신청 폼이면서 `isPaid(offer, items)`일 때만

실측(Playwright, dev 서버): 설문 8 → **4단계**(기본·방식 / 신청서 항목 / 디자인 / 검토·게시),
무료 신청 폼 8 → **7단계**. 참가비를 넣으면 `결제·환불`이 그 자리에 나타나고, 다시 0원으로
되돌리면 사라진다.

현재 위치는 인덱스가 아니라 **key**로 추적한다. 유료→무료 전환처럼 서 있던 단계가 사라지면
앞쪽의 가장 가까운 단계로 되돌아간다(첫 단계로 튀지 않는다). 실측으로 `결제·환불`에 있다가
0원으로 바꾸면 `가격과 정원`으로 이동하는 것을 확인했다.

**2. 어느 단계에서나 게시할 수 있다.** 마지막 단계 전까지 하단에 `게시하기`(게시된 폼이면
`변경 사항 저장`) 버튼이 있다. 누르면 `publishChecks`를 돌려 error가 있으면 게시하지 않고
**그 항목을 고칠 수 있는 단계로 이동**시키고 오류 문구를 띄운다.

이를 위해 `Check`에 `step?: CheckStep`을 추가했다. 단계가 숨겨질 수 있으므로 안내 문구의
`· 3단계` 같은 **번호 표기는 모두 단계 이름으로 교체**했다(`· 신청서 항목`). 코드 전체에서
하드코딩된 단계 번호를 제거했다.

**3. 자동 임시저장.** 기존에는 단계를 넘길 때만 저장돼서, 입력 중 탭을 닫으면 그 단계의
수정이 사라졌다. 이제 상태가 바뀐 뒤 **2초간 입력이 멈추면** `saveWizardAction`을 호출한다
(디바운스, `useEffect` + `setTimeout` cleanup). 단계 이동 시 즉시 저장하는 기존 동작은 유지.
하단 표시는 `입력하는 동안 자동으로 저장됩니다` → `저장 중…` → `HH:MM 자동 저장됨`.
실측: 제목만 입력하고 이동 없이 새로고침했을 때 값이 남아 있음을 확인했다.

주의: 폼은 언제나 `draft`/게시본 한 벌만 저장한다. **게시된 폼을 편집하면 자동 저장이
곧바로 라이브에 반영된다.** 게시본과 초안을 분리하는 버전 관리는 아직 없다(다음 후보).
