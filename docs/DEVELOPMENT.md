...
## 설문 기능 (구현 완료 2026-09-12)

아래 설계대로 구현됨. 실제로 만든 것:

- `types.ts`: `Questions.identity`(`'required' | 'none'`), `FormRecord.kind`(`'application' | 'survey'`). `db.ts`에 `forms.kind` 컬럼(기본 `'application'`)
- `createDraft(workspaceId, kind)`: 설문이면 단순 구조·무료·정원 없음·1인·알림 전체 비활성·익명으로 프리셋하고 `'응답'` 항목 1개를 함께 만든다. **엔진에 설문 전용 분기는 없다** — 프리셋된 필드 조합이 곧 설문이다
- `applyToForm`: `identity === 'none'`이면 이름·연락처 검증, 전화번호 중복 체크, 1인당 제한을 모두 건너뛴다
- `messaging.ts`: 전화번호가 없으면 발송하지 않는다
- `listCustomers`: `a.phone <> ''` 조건으로 익명 응답을 고객 DB에서 제외
- `lib/stats.ts`(신규): `summarizeAnswers()` 순수 함수. 선택형은 옵션별 집계(선택 안 된 옵션도 0%로 표시), 자유서술은 최근 20건 샘플. 비율은 전체가 아니라 **그 질문에 답한 사람 기준**
- `/admin/forms/[id]/results`(신규): 집계 화면. `getResults()`는 취소·만료를 제외한 응답만 읽는다. **신청 폼에서도 열린다**
- `publishChecks`·마법사 요약문이 익명일 때 "이름, 휴대폰" 대신 "익명 응답 · 질문 N개"로 말한다
- 공개 화면: 익명이면 이름·연락처 칸이 사라지고 버튼이 "응답 보내기"로 바뀐다

**Playwright 검증**: 설문 생성 시 프리셋(단순·무료) 적용 → 선택형/서술형 질문 추가 → 게시 체크 "익명 응답 · 질문 2개" → 익명 응답 3건 제출(이름 칸 없음 확인) → 집계에서 "매우 만족 2건·66.7%" 정확히 계산. **고객 DB 유입 없음도 별도 확인(111 → 111명 변화 없음).**

**남은 것**: 설문일 때 마법사에서 무의미한 단계(가격·정원, 결제·환불, 자동 알림)를 여전히 보여준다 — 숨기려면 `Wizard.tsx`의 인덱스 기반 스텝 렌더링을 데이터 기반으로 바꿔야 한다. 아래 "v1에서 빼는 것"도 그대로 미구현.

### 왜 하는가, 그리고 어디까지만 하는가

협회는 행사 신청과 별개로 설문을 받아 통계를 낸다. 지금은 그걸 구글폼으로 하게 되므로 **응답자 DB가 갈라진다** — "고객 DB가 한 곳에 모인다"가 차별화라면 설문도 같은 DB에 쌓이는 게 일관된다.

다만 **설문을 별도 제품이나 전면 마케팅 문구로 내세우지 않는다.** 이름에서 FORM을 뺀 이유와 같다. 신청 폼의 한 변형으로 조용히 지원한다.

### 설계: 새 제품이 아니라 기존 축의 조합

설문에 필요한 건 세 가지뿐이고, 두 개는 기존 필드로 이미 표현된다.

| 설문에 필요한 것 | 어떻게 | 신규 여부 |
|---|---|---|
| 항목 선택 없음 | `offer.structure = 'simple'` | 기존 |
| 정원·결제 없음 | `capacity = null`, `offer.free = true` | 기존 |
| **익명 응답** | `Questions.identity = 'none'` | **신규** |
| **집계 화면** | `/admin/forms/[id]/results` | **신규** |

그래서 `structure`에 4번째 값을 추가하거나 별도 엔진 경로를 만들지 않는다. 대신:

1. **`Questions.identity: 'required' | 'none'`** (기본 `'required'`) — 이름·연락처를 받을지 여부. 엔진이 분기하는 축은 이것 하나다
2. **`FormRecord.kind: 'application' | 'survey'`** — **표현 계층 전용.** 화면 문구("신청 폼" vs "설문"), 마법사에서 보여줄 단계, 기본 진입 탭만 결정한다. **엔진 로직은 `kind`를 보지 않는다** — 정원·결제·알림 동작은 전부 기존 필드(`structure`/`free`/`identity`)가 결정한다. 이렇게 해야 나중에 "익명 신청"이나 "실명 설문" 같은 조합이 나와도 구조가 버틴다

### 익명일 때 달라지는 것 (`identity: 'none'`)

- `applyToForm`: 이름·연락처 검증 건너뜀, 전화번호 기반 중복 체크·1인당 제한 건너뜀
- 알림 발송 없음 (보낼 번호가 없음)
- **고객 DB에서 제외** — `listCustomers`가 전화번호로 묶는데 익명 응답은 전부 빈 문자열로 한 덩어리가 되어버린다. 반드시 걸러야 한다
- 신청 내역 조회(`lookupApplications`)에 안 잡힘, QR·체크인 없음
- 중복 응답은 막지 않는다 (익명의 대가). 막아야 하면 실명 설문을 쓰는 게 맞다

### 집계 화면

`lib/stats.ts`의 순수 함수로 응답을 요약한다(필드 타입별):
- `select` / `multi` / `consent` → 옵션별 응답 수 + 비율 막대
- `text` / `textarea` / `email` → 집계 불가. 최근 응답 목록으로 보여줌

**신청 폼에서도 쓸 수 있게 만든다** — "신청자 중 60%가 인스타그램으로 알고 왔다" 같은 건 설문이 아니라 행사 운영에도 필요하다. 설문 전용 기능으로 가두지 않는다.

### v1에서 빼는 것

조건부 로직(분기 설문), 응답 기간 제한, 응답 수정, 중복 방지(쿠키·IP), 차트 시각화, 척도형 문항(1~5점). 척도형은 지금도 `select`로 대체 가능하다.
...
**R2는 이 결정과 함께 후보에서 빠졌다.** 비용(=$0.015/GB, egress 무료)만 보면 R2가 유리하지만, Cloudflare 공식 문서상 **Location Hint(`apac` 포함)는 "best effort이며 보장이 아니다"** — 성능 최적화용이다. 실제 소재지를 보장하는 Jurisdictional Restrictions는 **EU / FedRAMP / US만 제공하며 한국도 APAC도 없다.** 국내 보관을 이유로 DB를 서울에 두고 가장 민감한 파일(신분증 사본)을 보장 없는 위치에 두면 컴플라이언스 스토리에 정확히 최악의 지점에 구멍이 생긴다.

부수 효과로 구성이 단순해진다 — 벤더·콘솔·청구서가 하나, 서명 URL은 `createSignedUrl(path, 초)` 한 줄, 서빙 도메인이 `*.supabase.co`라 앱 도메인과 자동 분리되어 "별도 도메인 서빙" 보안 요구사항도 충족된다.

**확인 남은 것**: Supabase Storage 객체는 프로젝트 리전에 저장되지만 CDN 엣지 캐싱이 걸리면 캐시 사본이 국외 노드에 남을 수 있다. 엄격한 소재지 요건이 필요하면 서명 URL 기반 비공개 객체의 캐싱 동작을 Supabase에 직접 확인할 것.

### 누구의 정책인가 — 리전은 내 취향이 아니다

개인정보보호법 구조상 역할이 이렇게 갈린다:

- **신청자** = 정보주체
- **NODA.의 고객**(협회, 팝업 운영자) = **개인정보처리자** (수집 목적·방법을 정하는 주체)
- **NODA.** = **수탁자** (고객을 대신해 처리)

즉 국외이전 고지·동의 의무를 지는 쪽은 NODA.가 아니라 **고객**이다. 데이터의 주인은 고객의 고객이다.

**그런데 NODA.가 인프라를 하나만 고르면 그 선택이 모든 고객의 선택지를 제약한다.** 싱가포르만 제공하면 전 고객이 국외이전 절차를 떠안고, 그걸 받아들일 수 없는 고객(협회, 공공 연계)은 아예 못 쓴다. 워크스페이스별 리전 선택이 "고객이 정한다"의 진짜 답이지만 멀티리전 운영은 초기에 감당할 복잡도가 아니라, 엔터프라이즈 기능으로 미룬다.

**여기서 파생되는 제품 기회**: 고객은 개인정보처리방침에 위탁 사실·수탁자·처리 위치를 적어야 하는데 소규모 주최자는 대개 모르거나 귀찮아한다. 워크스페이스 설정에 보관 위치를 명시하고 **붙여넣을 수 있는 위탁 고지 문구를 자동 생성**해주면 컴플라이언스 부담이 기능이 된다. 협회·단체 타겟에 잘 맞을 것으로 보인다.

법적 요건의 구체적 내용(동의 필요 여부, 처리방침 고지로 갈음되는지 등)은 법령·전문가 확인 필요.

### 이식성 규칙 (가장 중요 — 결정이 바뀌어도 지킬 것)
...
**결정 기준이었던 것** (결론은 위 참고):
- 데이터 국내 보관이 필요하다 → **Supabase(서울)** ← 이쪽으로 결정됨
- 필요 없다 → **Neon + R2** (앱도 싱가포르 배포). 초기 비용이 거의 0에 수렴하고 브랜칭도 얻는다

Neon을 택했다면 **앱도 같은 리전에 배포해야 했다.** 피해야 할 조합은 "Vercel 서울 + Neon 싱가포르"다.
...
**퇴로 비대칭**: R2에서 나가는 건 egress 무료라 공짜, Supabase Storage에서 나가는 건 egress 요금이 붙는다. 소재지 요건 때문에 Supabase Storage를 택했으므로 이 비용은 감수하는 것이고, 그래서 아래 이식성 규칙 4번(객체 키만 저장)이 더 중요해진다.

**무료 티어만으로 실서비스 운영은 불가**. Supabase Free는 백업이 없고(Pro부터 일일 백업) 1주 비활동 시 프로젝트가 일시정지된다. 첫 유료 고객을 받는 순간 감당할 수 없는 조건이다. Pro $25에 파일 100GB + egress 250GB가 포함되므로, 워크스페이스별 용량 한도와 파기 정책을 붙이면 한동안 월 $25 고정으로 버틴다.
...
**~~잠재 버그~~ 해결됨 (2026-09-12)**: 커스텀 HTML에 `<input type="file">`이 있으면 sanitizer가 `type` 속성을 허용해 통과했고, `applyAction`이 모든 `f_` 값을 `.map(String)` 처리하므로 `"[object File]"`이 조용히 저장됐다. 지금은 `sanitizeFormHtml`의 `exclusiveFilter`가 파일 입력을 **텍스트로 강등하지 않고 통째로 제거**하고(강등하면 잘못된 값을 계속 조용히 받는다), `countFileInputs()`가 sanitize 이전 원본에서 개수를 세어 스캔 결과에 "파일 업로드 항목 N개는 아직 지원하지 않아 제외했습니다"로 알린다. 파일 입력만 있는 HTML은 전용 에러 메시지를 낸다. 실제 앱에서 검증 완료.
